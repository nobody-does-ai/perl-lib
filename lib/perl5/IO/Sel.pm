...
sub fileno
{
 ...
 if($f) {
   my $sub = $f->can("fileno");
   $sub = $sub || $f->can("_fileno");
   my $res = $sub->($f);
   ddx($res);
   $f=$res unless defined $res;
 };
 $f = $f->[0] if reftype($f) eq 'ARRAY';
...
IO::Sel - OO interface to the select system call
...
=head1 SYNOPSIS
    ...
    @ready = IO::Sel->new(@handles)->can_read(0);

=head1 DESCRIPTION

The C<IO::Sel> package implements an object approach to the system C<select>
...
Add the list of handles to the C<IO::Sel> object. It is these values that
will be returned when an event occurs. C<IO::Sel> keeps these values in a
...
C<IO::Sel> objects. C<TIMEOUT> is optional and has the same effect as
...
Here is a short example which shows how C<IO::Sel> could be used
