...
BEGIN {
  # During development inside Vim (make test), merge stderr into stdout so
  # everything lands in the quickfix buffer.  Gated on $ENV{VIM_TESTING} so
  # it never fires in production or on CPAN smokers.
  open(STDERR, ">&STDOUT") if $ENV{VIM_TESTING};
